...
def get_data_loaders(data_dir, loader, input_shape, val_input_shape, layout, oversampling, seed, batch_size, eval_batch_size, num_workers, benchmark, num_shards, global_rank):
    if loader == "synthetic":
        train_dataset = SyntheticDataset(scalar=True, shape=input_shape, layout=layout)
        val_dataset = SyntheticDataset(scalar=True, shape=val_input_shape, layout=layout)

    elif loader == "pytorch":
        x_train, x_val, y_train, y_val = get_data_split(data_dir, num_shards, shard_id=global_rank)
        train_data_kwargs = {"patch_size": input_shape, "oversampling": oversampling, "seed": seed}
        train_dataset = PytTrain(x_train, y_train, **train_data_kwargs)
        val_dataset = PytVal(x_val, y_val)
    else:
        raise ValueError(f"Loader {loader} unknown. Valid loaders are: synthetic, pytorch")

    train_sampler = DistributedSampler(train_dataset, seed=seed, drop_last=True) if num_shards > 1 else None
    val_sampler = None

    train_dataloader = DataLoader(train_dataset,
                                  batch_size=batch_size,
                                  shuffle=not benchmark and train_sampler is None,
    ...
    val_dataloader = DataLoader(val_dataset,
                                ...
                                shuffle=not benchmark and val_sampler is None,
                                sampler=val_sampler,
                                num_workers=num_workers,
                                pin_memory=True,
                                drop_last=False)
